...
def interact_with_env(env, state_dim, action_dim, max_action, device, args):
    setting     = f"{args.env}_{args.seed}"
    buffer_name = f"{args.buffer_name}"

    policy = network_ddpg.DDPG(state_dim, action_dim, max_action, device)
    if args.generate_buffer:
        policy.load(f"./models/behavioral_{setting}")

    replay_buffer = buffer.ReplayBuffer(state_dim, action_dim, device)
...
def train_bcq(state_dim, action_dim, max_action, device):
    # For saving files
    setting     = f"{args.env}_{args.seed}"
    buffer_name = f"{args.buffer_name}_{setting}"

    # Initialize policy
    policy = network_bcq.BCQ(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_behavioral", default = True, action ='store_true')
    ...
    if args.train_behavioral or args.generate_buffer:
        interact_with_env(env, state_dim, action_dim, max_action, device, args)
    else:
